...
class  Learnable_Loss(nn.Module):
    ...
    def __init__(self, ID_num=0):
        ...
        if args().learn_2dpose:
            self.loss_class['reg'].append('Segmentation')
        if args().learn_AE:
            self.loss_class['reg'].append('AE')

    def forward(self, outputs, view=None):
        if view is not None:
            perfix = f'{view}_'
        else :
            perfix = ''
            
        loss_dict = outputs[perfix+'loss_dict']
        ...
        outputs[perfix+'loss_dict'] = dict(loss_tasks, **loss_dict)
