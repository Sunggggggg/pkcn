...
default_cfg = {'save_dir':"./", 'vids':None, 'settings':[]} # 'put_org'
    ...
    def visulize_result(self, outputs, meta_data, show_items=['org_img', 'mesh'], vis_cfg=default_cfg, save2html=True, **kwargs):
        ...
        for vis_name in show_items:
            ...
            if vis_name == 'mesh' and outputs['detection_flag']:
                ...
                # per_img_verts_list = [vert[:1] for vert in per_img_verts_list]
                # mesh_trans = [trans[:1] for trans in mesh_trans if len(trans)>3]

                # print(per_img_verts_list)
                rendered_imgs = self.visualize_renderer_verts_list(per_img_verts_list, images=org_imgs.copy(), trans=mesh_trans)
                # rendered_imgs = self.visualize_renderer_verts_list(per_img_verts_list, images=np.ones_like(org_imgs)*255, trans=mesh_trans)
        ...
        # print(plot_dict['mesh_rendering_imgs']["figs"].shape)
