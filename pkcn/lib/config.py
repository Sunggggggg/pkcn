...
project_dir = currentfile.replace(os.sep + 'pkcn' + os.sep + 'lib' + os.sep + 'config.py','')
...
def parse_args(input_args=None):
    ...
    parser = argparse.ArgumentParser(description = 'PKCN: Part-attentive Kinematic Chain-based Regressor for 3D Human Modeling')
    parser.add_argument('--tab', type = str, default = 'PKCN_v1', help = 'additional tabs')
    ...
    # model settings
    model_group.add_argument('--backbone',type = str,default = 'dinov2',help = 'backbone model: resnet50 or hrnet')
    ...
    loss_group.add_argument('--segloss_epoch', default=10, type=int, help='point that stop to apply the segmenation loss')
    ...
    eval_group.add_argument('--eval_datasets',type = str,default = 'hi4d',help = 'whether to run evaluation')
    ...
    eval_group.add_argument('--calc_PVE_error',type = bool,default =True)
    ...
    dataset_group.add_argument('--dataset',type=str, default='hi4d' ,help = 'which datasets are used')
    ...
    smpl_group.add_argument('--smpl_joint_num',type = int,default = 24, help = 'joint number of SMPL model we estimate')
