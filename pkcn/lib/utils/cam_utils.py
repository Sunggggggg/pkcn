...
def adjust_intrinsic_for_pad_and_resize(K, pad_trbl, padded_shape, out_size=512):
    pad_top, pad_right, pad_bottom, pad_left = pad_trbl

    # Step 1: Padding (principal point 이동)
    K_adj = K.copy()
    K_adj[0, 2] += pad_left   # c_x
    K_adj[1, 2] += pad_top    # c_y

    # Step 2: Resize (스케일 팩터 적용)
    h_padded, w_padded = padded_shape[:2]
    scale_x = out_size / w_padded
    scale_y = out_size / h_padded

    K_adj[0, 0] *= scale_x    # f_x
    K_adj[1, 1] *= scale_y    # f_y
    K_adj[0, 2] *= scale_x    # c_x
    K_adj[1, 2] *= scale_y    # c_y
