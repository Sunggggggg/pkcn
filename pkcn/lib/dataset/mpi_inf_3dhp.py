...
class MPI_INF_3DHP(Image_base):
    def __init__(self, train_flag=True, validation=False, **kwargs):
        ...
        logging.info(
            'From file_paths with {} samples, removing subjects: {}, with {} samples left'.format(previous_sample_num,
                                                                                                  removed_subjects,
                                                                                                  len(self.file_paths)))
            ...
            for seq_i in seq_list:
                ...
                for j, vid_i in enumerate(vid_list):
                    ...
                    for frame_id in range(frame_num):
                        img_name = self.get_image_name(video_name,
                                                       frame_id)  # 'S{}_Seq{}_video{}_F{}.jpg'.format(user_i, seq_i, vid_i, frame_id)
                        ...
                        if _check_visible(kp2d, w=w, h=h) and frame_id % 10 == 1:
                            self.annots[img_name] = {'kp2d': kp2d, 'kp3d': kp3d, 'intrinsics': intrinsics,
                                                     'extrinsics': [R, T]}
    ...
    def get_image_name(self, video_name, frame_id):
        return video_name.strip('.avi').replace('/imageSequence', '').replace('/', '_') + '_F{:06d}.jpg'.format(
            frame_id)
