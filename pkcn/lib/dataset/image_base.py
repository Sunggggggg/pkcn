...
from utils.cam_utils import adjust_intrinsic_for_pad_and_resize

axis_angle_to_rotation_6d = lambda x : matrix_to_rotation_6d(axis_angle_to_matrix(x.reshape(-1, 3))).reshape(-1, 6)

def project_points(X, K):
    """
    X: [N, 3] (3D points, 카메라 좌표계)
    K: [3, 3] (intrinsic matrix)
    """
    X_homo = np.concatenate([X, np.ones((X.shape[0], 1))], axis=1)   # [N,4]
    x_proj = (K @ X_homo[:, :3].T).T    # [N,3]
    x_proj = x_proj[:, :2] / x_proj[:, 2:3]  # [N,2] (normalize by z)
    return x_proj

def draw_keypoints_on_image(image, kp2d, skeleton=None, color=(0,255,0)):
    img = image.copy()
    ...
    if skeleton is not None:
        for i, j in skeleton:
            pt1, pt2 = tuple(kp2d[i]), tuple(kp2d[j])
...
class Image_base(Dataset):
    def __init__(self, train_flag=True, regress_smpl = False):
        ...
        if args().learn_2dpose:
            self.heatmap_mapper = constants.joint_mapping(constants.SMPL_ALL_54, constants.SMPL_24)
            self.heatmap_generator = HeatmapGenerator(self.heatmap_res//2,len(self.heatmap_mapper))
        if args().learn_AE:
    ...
    def get_item_single_frame(self,index):
        ...
        # img_info = process_image(info['image'], info['kp2ds'], augments=(1., False, False), is_pose2d=info['vmask_2d'][:,0], multiperson=mp_mode)
        ...
        ### Global info. ###
        global_params = info['global_params']
        global_pose, global_shape = global_params[:72], global_params[72:]
        global_pose = axis_angle_to_rotation_6d(torch.from_numpy(global_pose)).numpy().reshape(144)  # [144]
        global_params = np.concatenate([global_pose, global_shape])  # [144+10]
        
        ### Intrinsic param. ###
        camExtrin = info['camExtrin']
        camIntrin = info['camIntrin']
        
        pad_trbl = compute_paddings_to_reach_aspect_ratio(info['image'].shape, 1.0)
        ...
        input_data = {
            ...
            'data_set': info['ds'],
            # Camera param.
            'camExtrin': torch.from_numpy(camExtrin).float(),
            'camIntrin': torch.from_numpy(camIntrin).float(),
            'crop_camIntrin': torch.from_numpy(crop_camIntrin).float()
            }
        
        ### Depth map ###
        if info.get('use_depth') :
            depth_img_info = process_image(info['depth_vis'], info['kp2ds'], augments=(scale, rot, flip), 
                                           is_pose2d=info['vmask_2d'][:,0], multiperson=mp_mode)
            depth_image, depth_image_wbg, depth_full_kps, depth_offsets = depth_img_info
            depth_image, depth_dst_image, depth_org_image = self.prepare_image(depth_image, depth_image_wbg, augments=(False, False))   # [512, 512]
            
            depth_dst_image = cv2.resize(depth_dst_image, (args().centermap_size, args().centermap_size), 
    ...
    def process_smpl_params(self, params, used_person_inds, augments=None, valid_mask_smpl=None):
        rot, flip = augments
        params_processed = np.ones((self.max_person,82), dtype=np.float32)*-10
