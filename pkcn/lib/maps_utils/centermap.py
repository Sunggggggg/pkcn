...
class CenterMap(object):
    ...
    def parse_centermap_heatmap_adaptive_scale_batch(self, center_maps):
        ...
        # Select the top K scores and their indices
        topk_scores, topk_inds = torch.topk(center_map_nms.reshape(b, c, -1), K)    # [B, C, hw] => [B, C, K]
        topk_inds = topk_inds % (h * w)
        topk_ys = torch.div(topk_inds, w, rounding_mode='floor').int().float()
        topk_xs = (topk_inds % w).int().float()
        
        # Sampling the top K scores and their indices
        topk_score, index = torch.topk(topk_scores.reshape(b, -1), K)               # [B, CK] => [B, K]
