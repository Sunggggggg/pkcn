...
class Trainer(Base):
    ...
    def train_log_visualization(self, outputs, loss, run_time, data_time, losses, losses_dict, epoch, iter_index, multi_view=None):
        ...
        losses_dict.update(outputs[perfix+'loss_dict'])
        ...
        if False :
            vis_ids, vis_errors = determ_worst_best(outputs[perfix+'kp_error'], top_n=3)
            ...
            if multi_view is None :
                ...
                for ds_name in set(flat_data_set):
                    save_name += '_{}'.format(ds_name)
                train_vis_dict = self.visualizer.visulize_result(outputs, outputs['meta_data'], show_items=['org_img', 'mesh', 'pj2d', 'centermap'],\
                    vis_cfg={'settings': ['save_img'], 'vids': vis_ids, 'save_dir':self.train_img_dir, 'save_name':save_name, 'verrors': [vis_errors], 'error_names':['E']})
    ...
    def train_epoch(self,epoch):
        ...
        for iter_index, meta_data in enumerate(self.loader):
            ...
            batch_size = len(meta_data["image"])
            meta_data["epoch"] = torch.tensor([[epoch]for _ in range(batch_size)])
    ...
    def test(self,epoch):
        logging.info('Test result on {} iters: '.format(epoch))
        for ds_name, val_loader in self.dataset_test_list.items():
            logging.info('Test on {}'.format(ds_name))
